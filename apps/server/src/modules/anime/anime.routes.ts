import { Router } from "express";
import { AnimeController } from "./anime.controller";
import { authenticate } from "../../middlewares/auth.middleware";

const router = Router();
const ctrl = new AnimeController();

router.get("/", ctrl.getList.bind(ctrl));
router.get("/search", ctrl.search.bind(ctrl));
router.get("/seasonal", ctrl.getSeasonal.bind(ctrl));
router.get("/top", ctrl.getTop.bind(ctrl));
router.get("/:id", ctrl.getById.bind(ctrl));
router.get("/:id/characters", ctrl.getCharacters.bind(ctrl));

// Tracking (auth required)
router.get("/me/list", authenticate, ctrl.getMyList.bind(ctrl));
router.put("/list/:animeId", authenticate, ctrl.upsertList.bind(ctrl));
router.delete("/list/:animeId", authenticate, ctrl.removeFromList.bind(ctrl));

export default router;
